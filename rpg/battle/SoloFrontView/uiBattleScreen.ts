...
class uiActionMenu extends Menu {
    private player:Character;
    private monster:Character;
    private _result:any;
    ...
    item() {
        var itemMenu = new uiItemMenu(this.player, this.monster, (item:Item) => {
            this._result.action = this.selections[this.selectionIndex].getAttribute('data-menu');
...
class uiItemMenu extends Menu {
    ...
    constructor(player, monster, callback) {
        ...
        this.itemList = Party.inventory.stacked();
        this.itemList.forEach((row:Array<Item>) => {
...
class uiItemRow extends Cozy.UiComponent {
